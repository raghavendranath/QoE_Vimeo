package com.android_workshop.raghav.qoe_vimeo.com.example.logger;

/**
 * Created by ragha on 11/3/2016.
 */
...
import com.android_workshop.raghav.qoe_vimeo.MainActivity;
...
public class Log {
    ...
    static {
        try {
            path = new File(Environment.getExternalStorageDirectory(), "QoE_Vimeo_Logs");
            if (!path.exists()) {
            ...
            mLogFile = new File(path.getAbsolutePath()//folder path
                    + File.separator
                    + "QoE_VM_logcat_" + MainActivity.time + ".txt" ); //file name
